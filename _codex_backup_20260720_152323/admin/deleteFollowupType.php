...
if((int)$used['total']>0){$_SESSION['followup_type_flash']='Follow-up type is in use and cannot be deleted.';}else{$stmt=mysqli_prepare($conn,'DELETE FROM followup_type_tbl WHERE id=?');mysqli_stmt_bind_param($stmt,'i',$id);mysqli_stmt_execute($stmt);mysqli_stmt_close($stmt);oecrm_audit($conn,'clients','delete','followup_type',$id,'Follow-up type deleted',$row);$_SESSION['followup_type_flash']='Follow-up type deleted.';}header('Location:manageFollowupType.php');exit;
