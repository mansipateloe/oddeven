<?php 
    ...
    if($projectId){
        $displayProject = "select * from taskTbl where projectId = $projectId";
        ...
        $getdepositRes = mysqli_num_rows($getdeposit);
         if($getdepositRes == 0){
             ...
             <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 189px;">Daily Work Hours</th>
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 189px;">Total hours</th>
            ...
            while($getReportRes = mysqli_fetch_assoc($getdeposit)){
                echo "<tr class='gradeA even' role='row'>";
                   ...
                   // print_r($rowproject); die();
                    ...
                    // <a href="editTask.php?edit='.$getReportRes["id"].'"><i class="fa fa-pencil" style="font-size:18px;"></i></a>&nbsp;&nbsp;
                    // <a href="deleteTask.php?delete='.$getReportRes["id"].'" onclick="return confirm(\'Are you sure you want to delete?\');"><i class="fa fa-trash-o" style="font-size:18px; color:red;"></i></a>
                    // </td>';
                echo "</tr>";
            }
            // $seconds = 0;
    ...
    else if($developerId !='' && $projectId == ''){
        $displayProject = "select * from taskTbl where developerId = $developerId";
        $getdeposit = mysqli_query($conn, $displayProject);
        $getdepositRes = mysqli_num_rows($getdeposit);
         if($getdepositRes == 0){
                echo '0';
