...
<?php if(strtotime($currentTime) > strtotime('10:30')): ?>
<script>
   $(function() {
       $("#myModal").modal();
   });
...
<?php if(isset($_POST['latesignIn'])) {
   ...
   if (mysqli_query($conn,$qry)){
       ...
       if (mysqli_query($conn,$qrysignIn)){
           ...
           echo "Signin time and reason not inserted succesfully.";
       }
   }else{
       echo "Reason not inserted succesfully.";
   }
   }
   ?>
<?php //elseif(isset($_POST['lunchin'])): ?>
<?php if(isset($_POST['lunchin'])) {
...
<?php if(isset($_POST['lunchout'])) {
   ...
   if(empty($rowLunch['lunchinTime'])){
       echo "You need to Lunch in first.";
   }else{
   ...
   ?>
<?php //elseif(isset($_POST['breakin'])): ?>
<?php if(isset($_POST['breakin'])) {
   ...
   $currentTime = json_encode($arrayCurrentTime);
   if(empty($rowBreakIn['signinTime'])){
       echo "You need to sign in first.";
   }else{
       $qry = "update workhoursTbl set breakinTime='$currentTime' where employeeId='$employeeId' and workDate='$currentDate'";
       if (mysqli_query($conn,$qry)){
           echo "Data inserted succesfully.";
       }else{
           echo "Data not inserted succesfully.";
   ...
   ?>
<?php //elseif(isset($_POST['breakout'])): ?>
<?php if(isset($_POST['breakout'])) {
   ...
   if(empty($rowBreak['breakinTime'])){
       echo "You need to Break in first.";
   }else{
       $qry = "update workhoursTbl set breakoutTime='$currentTime' where employeeId='$employeeId' and workDate='$currentDate'";
       if (mysqli_query($conn,$qry)){
           echo "Data inserted succesfully.";
       }else{
           echo "Data not inserted succesfully.";
...
<?php 
   if(isset($_POST['extraBreakin'])){
       ...
       if($arrlengthBreakInTime == $arrlengthBreakoutTime){
           ...
           echo "You need to breakout first.";
       }
   }
   ?>
<?php 
   if(isset($_POST['extraBreakout'])){
       ...
       $arrlengthBreakOutTime = count($resultBreakOutTime);
       if($arrlengthBreakInTime == $arrlengthBreakOutTime){
           echo "You need to breakin first";
       }else{
   ...
   ?>
<?php //elseif(isset($_POST['logoutBtn'])): ?>
<?php if(isset($_POST['logoutBtn'])) {
   ...
   $arrlengthBreakInTime = count($resultBreakInTime);
   $arrlengthBreakOutTime = count($resultBreakOutTime);
   /*print_r($arrlengthBreakInTime);
   print_r($arrlengthBreakOutTime);*/
   /*if($arrlengthBreakInTime == $arrlengthBreakOutTime){
       echo '<pre>'; print_r("match");die((__FILE__).'-->'.(__FUNCTION__).'--Line('. (__LINE__).')');
   }else{
       echo '<pre>'; print_r("not match");die((__FILE__).'-->'.(__FUNCTION__).'--Line('. (__LINE__).')');
   }*/
   if(!empty($lunchinTime) && empty($lunchoutTime)){
       //echo "You need to lunch out first.";
       ...
       /*echo "totalLunchTime".$totalLunchTime."<br>";*/
       ...
       /*echo $qry; die();
       echo '<pre>'; print_r("stop");die((__FILE__).'-->'.(__FUNCTION__).'--Line('. (__LINE__).')');*/
       if (mysqli_query($conn,$qry)){
           echo "Data inserted succesfully.";
       }else{
           echo "Data not inserted succesfully.";
                        ...
                        <div class="row">
                           <div class="col-sm-12">
                              <div class="table-responsive">
                                 <table class="table table-striped table-hover dataTable no-footer dtr-inline" id="dataTables-example" role="grid" aria-describedby="dataTables-example_info">
                                    <thead>
                                       <tr role="row" class="panel-heading">
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" >Date</th>
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending">Sign In</th>
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Lunch In</th>
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">Lunch Out</th>
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">Break IN</th>
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">Break Out</th>
                                          <th class="" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">Sign Out</th>
                                                         ...
                                                         if($result->num_rows > 0){
                                                             while($row = $result->fetch_assoc()){
                                                                 ...
                                                                 if(!empty($row['signinTime'])){
                                                                     ...
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     echo "<button type='submit' class='btn btn-outline btn-default' name='signin'>Sign in</button>";
                                                                     echo "</td>";
                                                                 }
                                                                 if(!empty($row['lunchinTime'])){
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     ...
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     echo "<button type='submit' class='btn btn-outline btn-primary' name='lunchin'>Lunch in</button>";
                                                                     echo "</td>";
                                                                 }
                                                                 if(!empty($row['lunchoutTime'])){
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     ...
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     echo "<button type='submit' class='btn btn-outline btn-success' name='lunchout'>Lunch out</button>";
                                                                     echo "</td>";
                                                                 }
                                                                 if(!empty($row['breakinTime'])){
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     ...
                                                                     if(!empty($row['breakoutTime']) && empty($row['signoutTime']))
                                                                     {
                                                                        ...
                                                                        {}else
                                                                        {
                                                                            if(count($breakInTimes)<2)
                                                                            {
                                                                                ...
                                                                                echo "<button type='submit' class='btn btn-outline btn-info' name='extraBreakin'>Break in</button>";
                                                                            }
                                                                     ...
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     echo "<button type='submit' class='btn btn-outline btn-info' name='breakin'>Break in</button>";
                                                                     echo "</td>";
                                                                 }
                                                                 if(!empty($row['breakoutTime'])){
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     ...
                                                                     if(!empty($row['breakoutTime']) && empty($row['signoutTime']))
                                                                     {
                                                                        ...
                                                                        {
                                                                         echo "<button type='submit' class='btn btn-outline btn-warning' name='extraBreakout'>Break out</button>";
                                                                        }
                                                                     }
                                                                     echo "</td>";
                                                                 }else{
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     echo "<button type='submit' class='btn btn-outline btn-warning' name='breakout'>Break out</button>";
                                                                     echo "</td>";
                                                                 }
                                                                 if(!empty($row['signoutTime'])){
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     ...
                                                                     echo "<td class='center-dashboard-block'>";
                                                                     echo "<button type='submit' class='btn btn-outline btn-danger' name='logoutBtn'>Logout</button>";
                                                                     echo "</td>";
                                                                 }
                                                                //  if(!empty($row['workHours'])){
                                                                //      echo "<td class='center-dashboard-block'>";
...
<script>
   ...
   $(document).ready(function() {
       ...
       /*$("#addextraBreakinBreakout").click(function(){
           $('#extraBreakin').show();
           $('#extraBreakout').show();
           $("#addextraBreakinBreakout").hide();
       });*/
   });
