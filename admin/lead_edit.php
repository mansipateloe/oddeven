        ...
        <div class="panel-body">
            <form action="action_update_lead.php" method="post">
                ...
                <div class="lead-form-grid">
                    ...
                    <div class="form-group"><label>Lead Source</label><select class="form-control" name="lead_source"><option value="">Select source</option><?php while($source=mysqli_fetch_assoc($sources)):?><option value="<?php echo (int)$source['id'];?>" <?php echo (int)$lead['lead_source']===(int)$source['id']?'selected':'';?>><?php echo oecrm_h($source['name']);?></option><?php endwhile;?></select></div>
                    <div class="form-group"><label>Mobile No. 1</label><input class="form-control" name="mobileno1" value="<?php echo oecrm_h($lead['mobile_no1']); ?>" inputmode="tel"></div>
                    <div class="form-group"><label>Mobile No. 2</label><input class="form-control" name="mobileno2" value="<?php echo oecrm_h($lead['mobile_no2']); ?>" inputmode="tel"></div>
                    <div class="form-group"><label>Status</label><select class="form-control" name="status"><?php foreach(['pending'=>'Pending','inprogress'=>'In Progress','complete'=>'Complete','close'=>'Closed'] as $value=>$label):?><option value="<?php echo $value;?>" <?php echo $lead['status']===$value?'selected':'';?>><?php echo $label;?></option><?php endforeach;?></select></div>
