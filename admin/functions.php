...
// example of usage
/*echo "564 : ".convertNum(10102)."<br>";
	echo "892 : ".convertNum(892);*/
?>

<?php
if (isset($_POST['convertNum'])) {
	echo convertNum($_POST['convertNum']);
}
?>
...
function get_exname_photo($uid)
{
	...
	if ($uid == "") {
		...
		$result = mysqli_query($conn, "select photo from employeestbl where emailid='$uid'");
...
function get_exname($uid)
{
	...
	if ($uid == "") {
		...
		$result = mysqli_query($conn, "select fname,lname from employeestbl where emailid='$uid'");
		if (mysqli_num_rows($result) > 0) {
			$row = mysqli_fetch_array($result);
...
function get_exname_phone($uid)
{
	...
	if ($uid == "") {
		...
		$result = mysqli_query($conn, "select phoneno from employeestbl where emailid='$uid'");
		if (mysqli_num_rows($result) > 0) {
			$row = mysqli_fetch_array($result);
			return $row['phoneno'];
		} else {
...
// 		$s = "";
// 	} else {
// 		$result = mysqli_query($conn, "select empcode from employeestbl where emailid='$uid'");
// 		if (mysqli_num_rows($result) > 0) {
// 			$row = mysqli_fetch_array($result);
// 			$s = $row['empcode'];
// 		} else {
...
{
	global $conn;
	$result = mysqli_query($conn, "select scoordinator from employeestbl where emailid='$emailid'");
	$row = mysqli_fetch_array($result);
	if (mysqli_num_rows($result) > 0) {
...
{
	global $conn;
	$sel = "select fname,lname from employeestbl where emailid='" . $uid . "'";
	$qry = mysqli_query($conn, $sel);
	$fet = mysqli_fetch_array($qry);
	return $fet['fname'] . " " . $fet['lname'];
}
...
function SetTimeFormatforEdit($time)
{
	$time_arr=explode(":",$time);
	for ($i=0; $i < count($time_arr); $i++) 
	{ 
		$cPart=$time_arr[$i];
		if($cPart<10 && strlen($cPart)==1)
		{
			$time_arr[$i]="0".$cPart;
		}
	}
	$return_time=implode(":",$time_arr);
	return $return_time;
}
