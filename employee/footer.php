	...
	<script src="../js/oecrm-ui.js?v=20260612-6"></script>
	<script>
	(function () {
		...
		var toggle = document.querySelector('.employee-menu-toggle');
		...
		if (toggle) {
			toggle.addEventListener('click', function () {
				body.classList.toggle('employee-sidebar-open');
			});
		}
		document.addEventListener('click', function (event) {
			if (window.innerWidth > 767 || !body.classList.contains('employee-sidebar-open')) return;
			if (event.target.closest('.sidebar') || event.target.closest('.employee-menu-toggle')) return;
			body.classList.remove('employee-sidebar-open');
		});
	})();
	</script>
	</body>
